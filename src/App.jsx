...
function App() {
  ...
  return (
    <>
      <h1>React hook</h1>
      {/* 빈 참조 객체를 원하는 돔요소에 연결 */}
      <input type="text" ref={inputRef} />

      {/* 버튼을 클릭하기 전까지는 input요소의 값을 확인 불가능하고 해당 input에 변경되는 value값은 리액트가 관리하지 않음 - 비제어 컴포넌트 */}
      <button onClick={() => console.log(inputRef.current.value)}>인풋 값 확인</button>
    </>
  );
}
export default App;
...
/*
  ...
  : 폼의 값 자체를 state에 담아둠
  : 장점 - 리액트가 실시간으로 폼 데이터의 변경사항을 추적가능
  : 단점 - 개발로직이 비제어 컴포넌트 방식에 비해서는 구조가 복잡하고 컴포넌트 재랜더링을 많이 발생 시킴
*/
