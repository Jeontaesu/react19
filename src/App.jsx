...
- 덜 중요한 데이터를 상태에 담으면 계속해서 컴포넌트를 재랜더링이 발생하기 때문에 비효율적
- 그렇다고 일반변수에 담으면 컴포넌트가 재호출될 때마다 해당 값이 사라짐
- useRef로 생성한 참조객체에 담아놓은 값은 컴포넌트가 재호출되더라도 값이 유지가됨
- 값이 유지가되는 것 뿐 해당 값이 변경되도 컴포넌트를 재랜더링 처리하지 않음
...
useRef 사용법
const 참조객체명 = useRef(초기값) --> {current: 초기값}
*/
